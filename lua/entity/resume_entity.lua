...
local ResumeEntity = {}
ResumeEntity.__index = ResumeEntity


function ResumeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ResumeEntity)
  self._name = "resume"
  self._client = client
...
function ResumeEntity:get_name()
  return self._name
end


function ResumeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ResumeEntity.new(self._client, opts)
end
...
function ResumeEntity:mark_deleted()
  self._deleted = true
end


function ResumeEntity:deleted()
  return true == self._deleted
end


function ResumeEntity:data_set(args)
  if args ~= nil then
...
function ResumeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ResumeEntity:match_set(args)
  if args ~= nil then
...
function ResumeEntity:match_get()
...
function ResumeEntity:stream(action, args, callopts)
...
---@param reqdata ResumeCreateData
---@param ctrl? table
---@return Resume
---@return string? err
function ResumeEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ResumeEntity:_run_op(ctx, post_done)
  local utility = self._utility
