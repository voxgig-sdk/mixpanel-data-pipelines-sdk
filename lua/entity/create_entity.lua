...
local CreateEntity = {}
CreateEntity.__index = CreateEntity


function CreateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CreateEntity)
  self._name = "create"
  self._client = client
...
function CreateEntity:get_name()
  return self._name
end


function CreateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CreateEntity.new(self._client, opts)
end
...
function CreateEntity:mark_deleted()
  self._deleted = true
end


function CreateEntity:deleted()
  return true == self._deleted
end


function CreateEntity:data_set(args)
  if args ~= nil then
...
function CreateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CreateEntity:match_set(args)
  if args ~= nil then
...
function CreateEntity:match_get()
...
function CreateEntity:stream(action, args, callopts)
...
---@param reqdata CreateCreateData
---@param ctrl? table
---@return Create
---@return string? err
function CreateEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function CreateEntity:_run_op(ctx, post_done)
  local utility = self._utility
