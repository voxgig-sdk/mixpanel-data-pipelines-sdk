...
local EditEntity = {}
EditEntity.__index = EditEntity


function EditEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EditEntity)
  self._name = "edit"
  self._client = client
...
function EditEntity:get_name()
  return self._name
end


function EditEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EditEntity.new(self._client, opts)
end
...
function EditEntity:mark_deleted()
  self._deleted = true
end


function EditEntity:deleted()
  return true == self._deleted
end


function EditEntity:data_set(args)
  if args ~= nil then
...
function EditEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EditEntity:match_set(args)
  if args ~= nil then
...
function EditEntity:stream(action, args, callopts)
...
---@param reqdata EditCreateData
---@param ctrl? table
---@return Edit
---@return string? err
function EditEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function EditEntity:_run_op(ctx, post_done)
  local utility = self._utility
