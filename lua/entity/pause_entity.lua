...
local PauseEntity = {}
PauseEntity.__index = PauseEntity


function PauseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PauseEntity)
  self._name = "pause"
  self._client = client
...
function PauseEntity:get_name()
  return self._name
end


function PauseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PauseEntity.new(self._client, opts)
end
...
function PauseEntity:mark_deleted()
  self._deleted = true
end


function PauseEntity:deleted()
  return true == self._deleted
end


function PauseEntity:data_set(args)
  if args ~= nil then
...
function PauseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PauseEntity:match_set(args)
  if args ~= nil then
...
function PauseEntity:match_get()
...
function PauseEntity:stream(action, args, callopts)
...
---@param reqdata PauseCreateData
---@param ctrl? table
---@return Pause
---@return string? err
function PauseEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function PauseEntity:_run_op(ctx, post_done)
  local utility = self._utility
