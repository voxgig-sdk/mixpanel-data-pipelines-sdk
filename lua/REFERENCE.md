# MixpanelDataPipelines Lua SDK Reference

Complete API reference for the MixpanelDataPipelines Lua SDK.
...
```lua
local create = client:Create(nil)
```
...
```lua
local result, err = client:Create():create({
})
```
...
```lua
local delete_pipeline = client:DeletePipeline(nil)
```
...
```lua
local result, err = client:DeletePipeline():create({
})
```
...
```lua
local result, err = client:Edit():create({
})
```
...
```lua
local result, err = client:Pause():create({
})
```
...
```lua
local pipeline_status = client:PipelineStatus(nil)
```
...
```lua
local results, err = client:PipelineStatus():list()
```
...
```lua
local resume = client:Resume(nil)
```
...
```lua
local result, err = client:Resume():create({
})
```
...
```lua
local retrieve_pipeline = client:RetrievePipeline(nil)
```
...
```lua
local result, err = client:RetrievePipeline():load({ project_id = 1 })
```
