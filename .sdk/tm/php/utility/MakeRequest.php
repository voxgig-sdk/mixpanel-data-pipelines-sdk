...
class MixpanelDataPipelinesMakeRequest
{
    public static function call(MixpanelDataPipelinesContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new MixpanelDataPipelinesResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new MixpanelDataPipelinesResponse($fetched);
        } else {
