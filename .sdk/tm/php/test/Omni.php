...
//    only - a typed MixpanelDataPipelinesContext there would make every ctx
...
final class MixpanelDataPipelinesOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new MixpanelDataPipelinesContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new MixpanelDataPipelinesSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new MixpanelDataPipelinesResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new MixpanelDataPipelinesError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new MixpanelDataPipelinesResponse($clean['response']);
    ...
    public static function err_from_map($m): ?MixpanelDataPipelinesError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MixpanelDataPipelinesError(is_string($code) ? $code : '', $msg);
    }
