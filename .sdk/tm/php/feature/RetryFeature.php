...
// MixpanelDataPipelines SDK retry feature
...
class MixpanelDataPipelinesRetryFeature extends MixpanelDataPipelinesBaseFeature
{
    ...
    public function init(MixpanelDataPipelinesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MixpanelDataPipelinesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MixpanelDataPipelinesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MixpanelDataPipelinesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
