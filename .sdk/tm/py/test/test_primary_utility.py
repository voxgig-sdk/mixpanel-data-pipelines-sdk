...
from projectname_sdk import MixpanelDataPipelinesSDK
from projectname_sdk.core.spec import MixpanelDataPipelinesSpec
from projectname_sdk.core.result import MixpanelDataPipelinesResult
from projectname_sdk.core.operation import MixpanelDataPipelinesOperation
from projectname_sdk.core.error import MixpanelDataPipelinesError
from projectname_sdk.feature.base_feature import MixpanelDataPipelinesBaseFeature
...
_runner = makeRunner(TEST_JSON_FILE, MixpanelDataPipelinesSDK.test(None, None))
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return MixpanelDataPipelinesError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_exists(self):
        client = MixpanelDataPipelinesSDK.test(None, None)
        utility = client._utility
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = MixpanelDataPipelinesResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = MixpanelDataPipelinesSDK.test(None, None)
        ...
        class TestInitFeature(MixpanelDataPipelinesBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(MixpanelDataPipelinesBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_live(self):
        ...
        live_client = MixpanelDataPipelinesSDK({
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = MixpanelDataPipelinesSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = MixpanelDataPipelinesSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = MixpanelDataPipelinesSDK.test(None, setup.get('a') or {})
    ...
    def test_operator_basic(self):
        def subject(vin):
            if not isinstance(vin, dict):
                vin = {}
            op = MixpanelDataPipelinesOperation(vin)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = MixpanelDataPipelinesSDK.test(None, setup.get('a') or {})
