...
describe('PipelineStatusDirect', async () => {
  ...
  test('direct-list-pipeline_status', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-pipeline_status', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MIXPANEL_DATA_PIPELINES_TEST_PIPELINE_STATUS_ENTID']
