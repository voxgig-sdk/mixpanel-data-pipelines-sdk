...
describe('RetrievePipelineDirect', async () => {
  ...
  test('direct-load-retrieve_pipeline', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-retrieve_pipeline', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MIXPANEL_DATA_PIPELINES_TEST_RETRIEVE_PIPELINE_ENTID']
