...
describe('RetrievePipelineEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'retrieve_pipeline.' + op, live)) return
    }
    ...
    let retrieve_pipeline_ref01_data = Object.values(setup.data.existing.retrieve_pipeline)[0] as any

    // LOAD
    const retrieve_pipeline_ref01_ent = client.RetrievePipeline()
    const retrieve_pipeline_ref01_match_dt0: any = {}
