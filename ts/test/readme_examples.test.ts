...
const SDK_NAME = 'MixpanelDataPipelinesSDK'
...
const TEST_SEED = {"entity":{"create":{"test01":{"id":"test01"}},"delete_pipeline":{"test01":{"id":"test01"}},"edit":{"test01":{"id":"test01"}},"pause":{"test01":{"id":"test01"}},"pipeline_status":{"test01":{"id":"test01"}},"resume":{"test01":{"id":"test01"}},"retrieve_pipeline":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+MixpanelDataPipelinesSDK\b/.test(code) ||
    /\bMixpanelDataPipelinesSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MixpanelDataPipelinesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MixpanelDataPipelinesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
