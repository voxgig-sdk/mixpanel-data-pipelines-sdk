...
type Args struct {
	Entity string         `json:"entity" jsonschema:"create | delete_pipeline | edit | pause | pipeline_status | resume | retrieve_pipeline"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.MixpanelDataPipelinesSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "mixpanel-data-pipelines_list",
		Description: "List records from MixpanelDataPipelines. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "mixpanel-data-pipelines_load",
		Description: "Load a single record from MixpanelDataPipelines. " +
...
func runOp(client *sdk.MixpanelDataPipelinesSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.MixpanelDataPipelinesSDK, name string) (sdk.MixpanelDataPipelinesEntity, error) {
	switch strings.ToLower(name) {
	case "create":
		return client.Create(nil), nil
	case "delete_pipeline":
		return client.DeletePipeline(nil), nil
	case "edit":
		return client.Edit(nil), nil
	case "pause":
		return client.Pause(nil), nil
	case "pipeline_status":
		return client.PipelineStatus(nil), nil
	case "resume":
		return client.Resume(nil), nil
	case "retrieve_pipeline":
		return client.RetrievePipeline(nil), nil
