...
class MixpanelDataPipelinesMakeError
{
    public static function call(?MixpanelDataPipelinesContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MixpanelDataPipelinesContext([], null);
        }
        $op = $ctx->op ?? new MixpanelDataPipelinesOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MixpanelDataPipelinesResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MixpanelDataPipelinesError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MixpanelDataPipelinesError('', $msg, $ctx);
        ...
        if ($err instanceof MixpanelDataPipelinesError) {
