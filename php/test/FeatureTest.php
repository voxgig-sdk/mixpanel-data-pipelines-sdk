...
// MixpanelDataPipelines SDK feature test
//
...
class FtCtrl extends MixpanelDataPipelinesControl
{
...
class FtClient
{
    ...
    private MixpanelDataPipelinesUtility $utility;

    public function __construct(MixpanelDataPipelinesUtility $utility, array $options)
    {
    ...
    public function get_utility(): MixpanelDataPipelinesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MixpanelDataPipelinesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MixpanelDataPipelinesContext $rootctx;

    // True when this SDK was generated with the named feature.
    public static function hasFeature(string $name): bool
    {
        if (self::$feature_config === null) {
            $config = MixpanelDataPipelinesConfig::shared_config();
            $f = $config['feature'] ?? [];
    ...
    public function __construct(
        ...
        $utility = new MixpanelDataPipelinesUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (MixpanelDataPipelinesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MixpanelDataPipelinesConfig::make_feature($name);
    ...
    private static function buildUrl(MixpanelDataPipelinesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MixpanelDataPipelinesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MixpanelDataPipelinesResponse($fetched);
        }
    ...
    private function fail(MixpanelDataPipelinesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MixpanelDataPipelinesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
