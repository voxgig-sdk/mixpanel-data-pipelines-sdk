...
class RetrievePipelineDirectTest extends TestCase
{
    public function test_direct_load_retrieve_pipeline(): void
    {
        $setup = retrieve_pipeline_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-retrieve_pipeline", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function retrieve_pipeline_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new MixpanelDataPipelinesSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
