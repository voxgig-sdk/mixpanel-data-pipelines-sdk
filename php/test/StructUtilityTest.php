...
// MixpanelDataPipelines SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = MixpanelDataPipelinesOmni::makeStructRunner(
    ...
    {
        return MixpanelDataPipelinesOmni::objmodel(MixpanelDataPipelinesOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return MixpanelDataPipelinesOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return MixpanelDataPipelinesOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(MixpanelDataPipelinesOmni::structfix($now, Struct::undef()))
                        !== json_encode(MixpanelDataPipelinesOmni::structfix($val, Struct::undef()))) {
