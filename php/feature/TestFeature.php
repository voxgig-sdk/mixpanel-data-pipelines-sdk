...
// MixpanelDataPipelines SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class MixpanelDataPipelinesTestFeature extends MixpanelDataPipelinesBaseFeature
{
    ...
    public function init(MixpanelDataPipelinesContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (MixpanelDataPipelinesContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = MixpanelDataPipelinesParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (MixpanelDataPipelinesContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(MixpanelDataPipelinesContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = MixpanelDataPipelinesParam::call($ctx, $k);
