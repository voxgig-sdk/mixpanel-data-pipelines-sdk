...
// MixpanelDataPipelines SDK RetrievePipeline entity
...
class RetrievePipelineEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new RetrievePipelineEntity($this->_client, $opts);
    }

    /**
     * @param RetrievePipeline|array $args RetrievePipeline data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = MixpanelDataPipelinesHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return RetrievePipeline|array The current RetrievePipeline data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of RetrievePipeline fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = MixpanelDataPipelinesHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of RetrievePipeline fields).
     */
    ...
    /**
     * Load a single RetrievePipeline.
     *
     * @param RetrievePipelineLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed RetrievePipelineLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return RetrievePipeline|array The loaded RetrievePipeline as an assoc-array at the
     *   SDK boundary; throws MixpanelDataPipelinesError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = MixpanelDataPipelinesHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
