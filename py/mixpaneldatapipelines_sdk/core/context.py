...
from mixpaneldatapipelines_sdk.core.control import MixpanelDataPipelinesControl
...
from mixpaneldatapipelines_sdk.core.helpers import get_ctx_prop, to_map
...
class MixpanelDataPipelinesContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = MixpanelDataPipelinesControl()
        ...
        if isinstance(sp, MixpanelDataPipelinesSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, MixpanelDataPipelinesResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, MixpanelDataPipelinesResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return MixpanelDataPipelinesOperation({})
        ...
        op = MixpanelDataPipelinesOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return MixpanelDataPipelinesError(code, msg, self)
