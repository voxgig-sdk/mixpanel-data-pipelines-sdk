...
class TestRetrievePipelineDirect:

    def test_should_direct_load_retrieve_pipeline(self):
        setup = _retrieve_pipeline_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-retrieve_pipeline", "live" if setup["live"] else "unit")
        if _skip:
...
def _retrieve_pipeline_direct_setup(mockres):
    ...
    live = env.get("MIXPANEL_DATA_PIPELINES_TEST_LIVE") == "TRUE"
    ...
    if live:
        ...
        client = MixpanelDataPipelinesSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
