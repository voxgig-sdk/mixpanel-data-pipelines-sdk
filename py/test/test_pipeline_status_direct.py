...
class TestPipelineStatusDirect:

    def test_should_direct_list_pipeline_status(self):
        setup = _pipeline_status_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-pipeline_status", "live" if setup["live"] else "unit")
        if _skip:
...
def _pipeline_status_direct_setup(mockres):
