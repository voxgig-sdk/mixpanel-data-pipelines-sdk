# MixpanelDataPipelines SDK test runner
...
class MixpanelDataPipelinesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MixpanelDataPipelinesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MixpanelDataPipelinesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MixpanelDataPipelinesTestRunner.getenv("MIXPANEL_DATA_PIPELINES_TEST_LIVE")
        override = MixpanelDataPipelinesTestRunner.getenv("MIXPANEL_DATA_PIPELINES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MixpanelDataPipelinesTestRunner.getenv(key)
        ...
        explain = MixpanelDataPipelinesTestRunner.getenv("MIXPANEL_DATA_PIPELINES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MIXPANEL_DATA_PIPELINES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MixpanelDataPipelinesTestRunner._test_control is not None:
            return MixpanelDataPipelinesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MixpanelDataPipelinesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MixpanelDataPipelinesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MixpanelDataPipelinesTestRunner.load_test_control()
...
def load_env_local():
    MixpanelDataPipelinesTestRunner.load_env_local()


def env_override(m):
    return MixpanelDataPipelinesTestRunner.env_override(m)


def entity_data(v):
    return MixpanelDataPipelinesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MixpanelDataPipelinesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MixpanelDataPipelinesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MixpanelDataPipelinesTestRunner.load_test_control()


def live_client_options():
    return MixpanelDataPipelinesTestRunner.live_client_options()


def live_delay_ms():
    return MixpanelDataPipelinesTestRunner.live_delay_ms()
