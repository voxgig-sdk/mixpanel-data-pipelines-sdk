# mixpanel-data-pipelines-cli

boru-driven command-line client **and** interactive REPL for the MixpanelDataPipelines
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mixpanel-data-pipelines-cli)
make build

# 2. See usage (words, entities, env vars)
./mixpanel-data-pipelines-cli --help
...
MIXPANEL_DATA_PIPELINES_BASE=https://api.example.com ./mixpanel-data-pipelines-cli --help

# 6. No arguments -> interactive REPL
./mixpanel-data-pipelines-cli
mixpanel-data-pipelines> /help
mixpanel-data-pipelines> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mixpanel-data-pipelines-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/mixpanel-data-pipelines-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `mixpanel-data-pipelines>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `create`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mixpanel-data-pipelines-cli`. |
...
(`./mixpanel-data-pipelines-cli <expr>`) and interactively (the REPL), and expressions compose the
