// Command MixpanelDataPipelines-cli is a CLI + REPL for the MixpanelDataPipelines SDK,
...
const prompt = "mixpanel-data-pipelines"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "create delete_pipeline edit pause pipeline_status resume retrieve_pipeline"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MIXPANEL_DATA_PIPELINES_APIKEY (api key), MIXPANEL_DATA_PIPELINES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MIXPANEL_DATA_PIPELINES_APIKEY carries the API key and
	// MIXPANEL_DATA_PIPELINES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MIXPANEL_DATA_PIPELINES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MIXPANEL_DATA_PIPELINES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMixpanelDataPipelinesSDK(opts)
